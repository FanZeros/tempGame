...
// UE: ComputeHistogramPositionFromLuminance
...
void PS()
{
    // Fragment position: x=[0,15] → 4 bins each, y=[0,127] → HDR128 row
    float fragX = floor(gl_FragCoord.x);
    float row = floor(gl_FragCoord.y);

    // This fragment is responsible for 4 bins
    ...
    // Iterate over all 128 pixels in this row of HDR128
    ...
    LOOP for (int i = 0; i < HDR128_WIDTH; i++)
    {
        ...
        // Map luminance to histogram bin position (float, 0-63)
        float fbin = ComputeHistogramPosition(lum);

        // Bilinear weight distribution to adjacent bins (matching UE)
        float binLow = floor(fbin);
        float fracW = fract(fbin);

        // Branchless: check if binLow or binLow+1 matches any of our 4 targets
