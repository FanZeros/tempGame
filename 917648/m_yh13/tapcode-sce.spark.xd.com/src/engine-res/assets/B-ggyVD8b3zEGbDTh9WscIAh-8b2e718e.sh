...
vec3 toAcesFilmic_HalfSafe(vec3 _rgb)
{
	// Reference(s):
	// - ACES Filmic Tone Mapping Curve
	//   https://web.archive.org/web/20191027010704/https://knarkowicz.wordpress.com/2016/01/06/aces-filmic-tone-mapping-curve/
	hvec3 rgbh = hvec3_init(_rgb.x, _rgb.y, _rgb.z);
	hfloat aa = 2.51f;
	hfloat bb = 0.03f;
	hfloat cc = 2.43f;
	hfloat dd = 0.59f;
	hfloat ee = 0.14f;
	rgbh = saturate( (rgbh*(aa*rgbh + bb) )/(rgbh*(cc*rgbh + dd) + ee) );
	return vec3(rgbh.x, rgbh.y, rgbh.z);
}
...
hfloat Square(hfloat x)
{
    return x * x;
}

hvec2 Square(hvec2 x)
{
    return x * x;
}

hvec3 Square(hvec3 x)
{
    return x * x;
}

hvec4 Square(hvec4 x)
{
    return x * x;
}

hfloat Pow5 (hfloat x)
{
    return x*x * x*x * x;
}

hfloat DecodeRGBAToFloat(hvec4 rgba)
{
	return dot(rgba * 255.0, hvec4_init(1.0, 256.0, 65536.0, 16777216.0)); // rgba 范围是0.0~1.0
}
...
{
    float temp = mix(0.0 - Contrast, 1.0 + Contrast, In);
