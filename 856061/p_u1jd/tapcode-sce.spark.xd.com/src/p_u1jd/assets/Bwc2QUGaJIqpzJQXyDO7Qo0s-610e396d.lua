...
local okTransport, ProxyTransport = pcall(require, "sdk.gamealgo.ProxyTransport")
...
function GameAlgo.FetchConfig(callback)
    ...
    httpRequest("POST", "/v1/config", request, function(error, config, response)
        if error then
            local detail = ""
            if response and response.body and response.body ~= "" then
                detail = " body=" .. tostring(response.body)
            end
            log("config fetch failed: " .. tostring(error) .. detail)
