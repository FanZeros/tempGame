...
-- 仅编辑器 -editor_debug_tapcode 才认 direct_host，避免正式包用命令行把整局重定向到任意地址。
local function resolveDirectConnect(ext)
    if not (HasAppArg and HasAppArg("editor_debug_tapcode")) then
        return nil, nil
    end
    ext = ext or {}
    local host, port = ext.direct_host, ext.direct_port
    if GetAppArgv then
        local argHost = GetAppArgv("direct_host")
        if argHost and argHost ~= "" then
            host, port = argHost, GetAppArgv("direct_port")
        end
    end
    port = tonumber(port)
    if type(host) == "string" and host ~= "" and port and port >= 1 and port <= 65535 then
...
local function buildMatchParams(mp, projectVersion, opts)
    ...
    if opts.immediatelyStart ~= nil then
        immediatelyStart = opts.immediatelyStart and true or false
    end
    ...
    local host, port = resolveDirectConnect(ext)
    if host then
        matchInfo.direct_host = host
        matchInfo.direct_port = port
    end
...
local function attachBridge()
    ...
    lobbyBridge:setCallHandler(function(action, argsJson, _bridgeReqId)
        ...
        elseif action == "startGame" then
            runAction(rid, action, function()
                local room = resolveRoomParams(a)
                local mp = readMultiplayer()
                -- 未开直连时不传 matchInfo，StartGame 仍只发 { mode, immediately_start=true }。
                if resolveDirectConnect(mp.match_info) then
                    room.matchInfo = buildMatchParams(mp, projectVersion(), {
                        mapName = room.mapName,
                        mode = room.mode,
                        immediatelyStart = true,
                    }).matchInfo
                end
                return lobbyMgr_:StartGame(withResult(rid, room))
            end)
