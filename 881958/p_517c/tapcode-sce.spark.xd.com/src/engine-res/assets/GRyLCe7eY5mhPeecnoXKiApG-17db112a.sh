...
// lerp 是 HLSL 名字，GLSL 用 mix。bgfx 把 HLSL / PSSL / SPIRV / METAL 归到同一套 HLSL 风味
// 名字体系，那一族在下方的大块里自带 lerp 的显式展开式、并用它反向实现 mix。
// BGFX_SHADER_LANGUAGE_GLSL 非 0 当且仅当本次输出是 GLSL / ESSL 且不经 HLSLCC。
...
#	define BGFX_CONFIG_MAX_BONES 32
	...
	// 只包初始化列表的数组构造，声明部分由调用方自己写。GLSL 只认构造式 ctor[](...)，HLSL 只认
	// 花括号。ARRAY_BEGIN 把声明类型与构造类型合成同一个参数，而带精度限定的类型（hvec3 即
	// highp vec3）不允许出现在构造位置，所以这里只收构造类型，调用方传 hvecN_init 这类无精度别名。
	#if BGFX_SHADER_LANGUAGE_GLSL
	#	define ARRAY_INIT_BEGIN(_ctor) _ctor[](
	#	define ARRAY_INIT_END() )
	#else
	#	define ARRAY_INIT_BEGIN(_ctor) {
	#	define ARRAY_INIT_END() }
	#endif // BGFX_SHADER_LANGUAGE_GLSL

	// 标量 splat。HLSL 的单标量数值构造非法（float3(x) 报 X3014），只能走 swizzle；GLSL ES 反过来
	// 不接受标量 swizzle，只能走构造式。两种写法都只求值一次，结果精度随操作数。
	#if BGFX_SHADER_LANGUAGE_GLSL
	#	define hvec2_splat(_x) hvec2_init(_x)
	#	define hvec3_splat(_x) hvec3_init(_x)
	#	define hvec4_splat(_x) hvec4_init(_x)
	#else
	#	define hvec2_splat(_x) (_x).xx
	#	define hvec3_splat(_x) (_x).xxx
	#	define hvec4_splat(_x) (_x).xxxx
	#endif // BGFX_SHADER_LANGUAGE_GLSL
	...
	vec4 vec4_splat(float _x) { return vec4(_x, _x, _x, _x); }
	 ...
	 || BGFX_SHADER_LANGUAGE_METAL
		...
		// 判据要带上 PSSL / SPIRV / METAL，跟下面几处同类判据保持一致：'##' 那支是 dx9 专用，
		// 它会抑制相邻参数的宏展开，粘出的是字面宏名而不是值——例如 clusters.sh 里
		// REGISTER(u, SAMPLER_CLUSTERS_LIGHTGRID) 会变成 register(uSAMPLER_CLUSTERS_LIGHTGRID)，
		// glslang 报 "expected register number after register type"。
		...
		#	if BGFX_SHADER_LANGUAGE_HLSL > 300 || BGFX_SHADER_LANGUAGE_PSSL || BGFX_SHADER_LANGUAGE_SPIRV || BGFX_SHADER_LANGUAGE_METAL
			...
			// 梯度形参必须是 h 前缀全精度：SPIR-V 要求 Grad 操作数 32 位，形参写 vec2 会把调用点
			// 已是 hvec2 的实参窄化回 half2。_coord 不受此限。
			hvec4 bgfxTexture2DGrad(BgfxSampler2D _sampler, vec2 _coord, hvec2 _dPdx, hvec2 _dPdy)
			{
		...
		// 只在 iOS Metal 现编启用，其他端跳过。
		// 解决 IOS Shader 编译。GLSL方言 -> HLSL ---glslang--> SPIR-V --SPIRV-Cross--> MSL 中
		// glslang 的 HLSL 内建表 将 length / distance 的返回类型硬写成全精度 float
		// 若有参数设置为 half 或 min16float，SPIR-V 里就会出现 "结果 %float、操作数 %half" 的类型不匹配， SPIR-V 校验拒绝。
		// 这里通过用 dot + sqrt 重写，绕开内置函数，保证返回类型与参数类型一致，避免 SPIR-V 校验拒绝。
		// 已测试：metal(hlslcc on/off), dx11(hlslcc on), android(hlslcc on/off), webgl(hlslcc on/off)。
		// 入参 vec* 是 half，平方过 65504 会 Inf；先拷到 h 前缀全精度再算，返回时变回 half。
		float length(vec2 _v) { hvec2 v = _v; return sqrt(dot(v, v)); }
		float length(vec3 _v) { hvec3 v = _v; return sqrt(dot(v, v)); }
		float length(vec4 _v) { hvec4 v = _v; return sqrt(dot(v, v)); }
		float distance(vec2 _a, vec2 _b) { hvec2 a = _a; hvec2 b = _b; return sqrt(dot(a - b, a - b)); }
		float distance(vec3 _a, vec3 _b) { hvec3 a = _a; hvec3 b = _b; return sqrt(dot(a - b, a - b)); }
		float distance(vec4 _a, vec4 _b) { hvec4 a = _a; hvec4 b = _b; return sqrt(dot(a - b, a - b)); }

		// 求导内建编成 OpFwidth / OpDPdx / OpDPdy，SPIR-V 禁止 fp16 操作数，而本族把 float / vec*
		// 降成 half，于是报 "Result type component width must be 32 bits"。只能真的用 32 位算。
		// 决定因素是实参：结果存进 hvec2 也不管用，实参是 vec2（=half2）就非法。
		// 用独立名字而非重载 ddx / ddy：某些档位上 hvec* 与 vec* 是同一类型，同名重载会变成自我
		// 调用（D3DCompile 报 X3500 recursive functions not allowed）。形参只收 h 前缀类型，
		// 实参是 half 时隐式提升即可。
		hfloat bgfxDdx(hfloat _x) { return ddx(_x); }
		hvec2  bgfxDdx(hvec2  _x) { return ddx(_x); }
		hvec3  bgfxDdx(hvec3  _x) { return ddx(_x); }
		hvec4  bgfxDdx(hvec4  _x) { return ddx(_x); }
		hfloat bgfxDdy(hfloat _y) { return ddy(_y); }
		hvec2  bgfxDdy(hvec2  _y) { return ddy(_y); }
		hvec3  bgfxDdy(hvec3  _y) { return ddy(_y); }
		hvec4  bgfxDdy(hvec4  _y) { return ddy(_y); }

		// 覆盖上方的原始定义，把实参提到全精度。dFdy 的取负约定沿用原样。
		#undef  dFdx
		#undef  dFdy
		#define dFdx(_x) bgfxDdx(_x)
		#define dFdy(_y) bgfxDdy(-(_y))

		// 实参已是 h 前缀全精度，直接用内建；取了 abs，dFdy 的取负差异不影响结果。
		float fwidth(float _x) { hfloat h = _x; return abs(ddx(h)) + abs(ddy(h)); }
		vec2  fwidth(vec2  _x) { hvec2  h = _x; return abs(ddx(h)) + abs(ddy(h)); }
		vec3  fwidth(vec3  _x) { hvec3  h = _x; return abs(ddx(h)) + abs(ddy(h)); }
		vec4  fwidth(vec4  _x) { hvec4  h = _x; return abs(ddx(h)) + abs(ddy(h)); }
#endif // BGFX_SHADER_LANGUAGE_METAL
