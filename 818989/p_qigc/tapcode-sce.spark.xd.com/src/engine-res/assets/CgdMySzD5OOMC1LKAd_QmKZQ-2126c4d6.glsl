...
#ifdef COMPILEPS
#ifdef BGFX_SHADER
uniform hvec4 u_TonemapExposureBias;
uniform hvec4 u_TonemapMaxWhite;
#define cTonemapExposureBias u_TonemapExposureBias.x
#define cTonemapMaxWhite u_TonemapMaxWhite.x
#else
uniform hfloat cTonemapExposureBias;
uniform hfloat cTonemapMaxWhite;
#endif
#endif
...
void PS()
{
    ...
    #ifdef ACES_APPROX
    vec3 color = toAcesFilmic(texture2D(sDiffMap, vScreenPos).rgb);
    gl_FragColor = vec4(color, 1.0);
    #endif
}
