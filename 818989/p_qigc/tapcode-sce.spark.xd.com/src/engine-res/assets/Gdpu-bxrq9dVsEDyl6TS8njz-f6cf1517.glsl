...
#ifdef COMPILEPS
#ifdef BGFX_SHADER
uniform hvec4 u_AutoExposureAdaptRate;
uniform hvec4 u_AutoExposureLumRange;
uniform hvec4 u_AutoExposureMiddleGrey;
uniform hvec4 u_HDR128InvSize;
uniform hvec4 u_Lum64InvSize;
uniform hvec4 u_Lum16InvSize;
uniform hvec4 u_Lum4InvSize;
...
#else
uniform hfloat cAutoExposureAdaptRate;
uniform hvec2 cAutoExposureLumRange;
uniform hfloat cAutoExposureMiddleGrey;
uniform hvec2 cHDR128InvSize;
uniform hvec2 cLum64InvSize;
uniform hvec2 cLum16InvSize;
uniform hvec2 cLum4InvSize;
#endif
...
void PS()
{
    ...
    float prevAdaptedLum = texture2D(sDiffMap, vTexCoord).g;
    float lum = clamp(texture2D(sNormalMap, vTexCoord).r, cAutoExposureLumRange.x, cAutoExposureLumRange.y);
    if (prevAdaptedLum <= 0.0)
        prevAdaptedLum = lum;
    float adaptedLum = prevAdaptedLum + (lum - prevAdaptedLum) * (1.0 - exp(-cDeltaTimePS * cAutoExposureAdaptRate));
    // Output same format as histogram approach:
    //   .r = exposureScale (BloomHDR reads this)
    //   .g = adaptedLum (temporal feedback)
    float targetExposure = adaptedLum / 0.18;
    float exposureScale = 1.0 / max(targetExposure, 1e-10);
    gl_FragColor = vec4(exposureScale, adaptedLum, lum, 0.0);
    #endif
